...
#         centroid2word[idx_centroid].append(w)
    ...
    X_centroids = np.zeros( (len(X), num_clusters), dtype="float32" )
    counter = 0
    for s in X:
        X_centroids[counter] = create_bag_of_centroids(s.split(), word_centroid_map )
        counter += 1
    print '%d feature vectors created based on training data' % (counter)
    
    X_train_centroids = X_centroids[:lengthTrain]
    X_test_centroids = X_centroids[lengthTrain:]
    
    
    vectorizer = CountVectorizer(tokenizer = None,    \
                             preprocessor = None, \
                             stop_words = None,   \
                             max_features = 20000, ngram_range =(1,2), binary=True)
...
#     clf = SVC(kernel='linear')
    ...
    clf = clf.fit( X_train_centroids, Y_train)
    ...
    y_pred = clf.predict(X_test_centroids)
